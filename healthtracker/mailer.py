...
from healthtracker.db import Patient
...
def send_admin_login():
    ...
    patient = Patient.query.filter_by(email="isaachodes@gmail.com").first()

    email_text = "click to login: http://{0}/admin?auth_token={1}".format(app.config["HOST_NAME"], patient.auth_token)
...
def send_login_email(patient):
    ...
    email_text = "Click to login: http://{0}/tracker?auth_token={1}".format(app.config["HOST_NAME"], patient.auth_token)
...
def send_status_update_email(patient):
    api_endpoint = "https://api.mailgun.net/v2/healthtracker.mailgun.org/messages"
    api_key = app.config["MAILGUN_API_KEY"]
...
def send_confirmation_email(patient):
    api_endpoint = "https://api.mailgun.net/v2/healthtracker.mailgun.org/messages"
    api_key = app.config["MAILGUN_API_KEY"]
    from_email = "HealthTracr <hello@healthtracker.mailgun.org>"
...
If you didn't signed up for HealthTracr.com, please ignore this email""".format(app.config["HOST_NAME"], patient.auth_token)
...
def send_approval_email(patient):
    api_endpoint = "https://api.mailgun.net/v2/healthtracker.mailgun.org/messages"
    api_key = app.config["MAILGUN_API_KEY"]
    from_email = "HealthTracr <hello@healthtracker.mailgun.org>"
...
Founder, HealthTracr""".format(app.config["HOST_NAME"], patient.auth_token)
